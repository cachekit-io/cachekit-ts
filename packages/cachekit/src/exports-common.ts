/**
 * Platform-neutral public exports shared by BOTH entrypoints (Node index.ts
 * and workers/index.ts). Everything here must be loadable on Node AND
 * Cloudflare Workers — no node:* builtins, no NAPI, no ioredis, no
 * prom-client in this module graph (CI-enforced by the workers bundle
 * guard). Platform-specific exports (redis, NAPI/wasm bindings, the
 * invalidation channel, each entry's createCache) live in the entrypoints.
 */
...
// ============ Backends (fetch-based, platform-neutral) ============
export {
  cachekitio,
