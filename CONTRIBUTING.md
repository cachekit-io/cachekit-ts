...
- CI's **Verify bounded floor-pins stay in-major** step declares the bounded pins and their upper-bound majors, and fails the security job unless the file still matches that set exactly. It catches widening, bound deletion, and any rewrite into a range shape it cannot parse (a `||` union, or a bound like `<12.1` that admits part of the next major) — anything unrecognised drops out of the comparison and fails closed. It reads the file line by line, so it also requires every `overrides` entry to sit on one line with its value quoted. This is the layer that holds when a vulnerability-remediation PR or a hand edit rewrites one of the declared entries. A genuinely necessary cross-major security fix — as `uuid` needed when no in-major fix existed — stays possible: CI goes red, you read why, and you remove the bound and its entry in that step in a commit that says so. Red is the discovery channel, not a wall.
